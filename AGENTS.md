...
### v1 のUIプリミティブは Ark UIを使う

`src/routes/v1/` 以下で新しく作るメニュー・ダイアログ・ポップオーバー等の
コンポーネントプリミティブは **Ark UI (`@ark-ui/solid`)** を使う。
Kobalte (`@kobalte/core`) は v0 の既存実装にだけ残っているもので、v1 の新規実装へ
持ち込まない。見た目の一次情報は Penpot の streets ファイル。
...
Playwright はテスト開始前に `pnpm build` を完了させ、その成果物を `vite preview` で配信します。開発サーバーのオンデマンド変換は使わないため、**新しいルートを使う spec に暖機処理を足す必要はありません**。先頭の spec だけが不安定な場合も、ルート暖機やタイムアウト延長ではなく、アプリやリレーの準備完了条件を調べてください。
...
通常の機能実装は **GitHub Issue + 実装前のチャット確認**を仕様とする。優先度整理、
既存仕様に沿う実装、小さい修正のために別の仕様書や計画書を増やさない。

別の仕様書が必要なのは、秘密鍵・アカウント境界、読み取り層の責務分割、永続形式など、
複数 Issue にまたがり後から戻しにくい設計を新しく決める場合だけ。作る場合も仕様だけを
残し、作業手順だけの計画書はコミットしない。恒久的な決定と却下理由は ADR、実装で
得た知見は `docs/design/read-layer-followups.md` に置き、同じ説明を複製しない。
