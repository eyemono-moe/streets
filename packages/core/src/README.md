...
| ディレクトリ | 役割 |
|---|---|
| `read/` | 読み取り層。接続、購読、store、セクション、コアレッサ |
| `write/` | 署名して publish する経路と楽観挿入の検証 |
| `signer/` | NIP-07 と NIP-46。秘密鍵はここにも入らない |
| `relay/` | 1 リレー分の WebSocket |
| `nostr/` | イベントの組み立て（`build/`）、パース、NIP-19 |
| `deck/` | カラム定義、`Source` への解決、通知フィルタ、カラム警告 |
| `view/` | レンダラ登録、描画窓、集計、整形 |
| `moderation/` | NIP-51 ミュートリストの解読と変更 |
| `settings/` | 端末設定とリレーリストの状態 |
| `solid/` | カラムとスレッドの購読を Solid のリソースにする |
