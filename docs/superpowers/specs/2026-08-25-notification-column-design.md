...
`TIMELINE_KINDS` が kind:16（汎用リポスト）を外しているのと同じ理由。kind:16 の対象は kind:1 以外（長文記事など）であり、**v1 がまだ作れない種類のコンテンツ**を指す。自分の kind:1 を kind:16 でリポストすることは規約上できなくはないが、実際には起きない。

長文に対応したときに `NOTIFICATION_KINDS` へ 1 つ足せば乗る。レンダラ（`Repost.tsx`）は既に kind:16 で登録済み。
