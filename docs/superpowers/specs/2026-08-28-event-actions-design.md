...
一覧は SectionReader の表示上限と同じ直近 200 件に制限する。リレーエコーを受けた
最初のカラムを契機に削除すると、まだ受けていない別カラムから一瞬消えるため、成功時
には即削除しない。各カラムは購読本体と id で重複排除し、長時間のセッションでも
保持量と照合量が増え続けないよう上限で収束させる。
上限への切り詰めは publish 成功の確定時に行う。未確定イベントの挿入時には既存の
成功イベントを追い出さず、その publish が全滅した場合は以前の一覧を保つ。
