...
| 相 | 実測 | 割合 |
|---|---|---|
| ① `kind:3`（インデクサ 4 本） | 1996.6 ms | 57% |
| ② `kind:10002`（全フォロイー） | 1496.7 ms | 43% |
| 計測外 | 1.7 ms | — |

**② は支配的ではなかった。** ② をキャッシュで消しても warmUp は約 1998 ms 残る。

**それでもこのスライスはこのまま進める。** ADR-0011 の 2 秒はこちらで置いた数字であり、基盤より先に最適化する理由にはならない。キャッシュポリシーの枠組みと ② の 1.5 秒はそれ自体で価値がある。

**両相とも `collect()` の「全 URL が settle するまで待つ」に縛られている**（最も遅い 1 本が所要時間を決める）ことが分かったが、**どのリレーがどれだけかかったかは測っていない**ので、原因を接続失敗に帰することはできない。この 2 点は [#262](https://github.com/eyemono-moe/streets/issues/262)（リレーごとの所要時間の計測）と [#263](https://github.com/eyemono-moe/streets/issues/263)（完了条件の見直し）として切り出した。**#262 を先に行うこと** —— 遅い相手を知らずにデッドラインを決めても、根拠のない定数が増えるだけになる。
