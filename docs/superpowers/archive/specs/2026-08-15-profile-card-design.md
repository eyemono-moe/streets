...
### 5.2 この節は実装後に 2 点ひっくり返った（2026-08-15 追記）

**掛ける場所は「名前が出るところすべて」になった。** 実物を触った結果、リプライ先・リポスト/リアクションの見出し・リアクション一覧の押した人・本文中の言及でカードが出ないのは一貫性を欠くという判断。7 箇所とも `<Profile>` を通るので、`<Profile>` 自体にホバーを入れて 1 箇所で揃えた。

**この節が挙げていた入れ子の理由は誤りだった。** `ProfileCard` は `<Profile>` を使わず生の文字列を描いているので、カード自身の名前からは再帰しない。実際に残る経路は「カード → 自己紹介文 → `nostr:npub` の言及 → `<Profile>` → カード」の 1 本だけで、こちらは `ProfileHoverSuppressedProvider`（`src/routes/v1/profile-hover-context.ts`）で自己紹介文の下だけホバーを止めて塞いだ。**カード全体ではなく自己紹介文だけを包む** —— ルートを provider にするとコンポーネントの戻り値が DOM 要素でなくなり、直接呼ぶテストが全部壊れる（実際に壊した）。

**トリガーは押せる見た目にした。** 押しても今は何も起きないが、[#205](https://github.com/eyemono-moe/streets/issues/205) でクリックしてユーザーカラムを開く挙動にする予定であり、それまで見た目を分ける理由が無い（v0 の `EmbedUser` も同じ見た目）。`bg-transparent` は引き続き要る —— こちらは見た目の方針ではなく、`<button>` の UA 既定背景を打ち消すためのもの。
