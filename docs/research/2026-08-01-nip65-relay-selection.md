...
- **`relay-jp.nostr.wirednet.jp` は `max_subscriptions: 8`。** マルチカラムのクライアントにとってこれは厳しい。streets は 10 カラム（ADR-0011）を想定しており、1 カラムが 1 リレーに 1 購読でも上限に当たる。**既定リレーに入れる場合、購読数の予算がリレーごとに違うことを扱う必要がある。**
