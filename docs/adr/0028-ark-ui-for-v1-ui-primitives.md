...
v0 は `@kobalte/core` でヘッドレス UI（`Popover` / `Progress` など）を組んでいる。今回そのために `@ark-ui/solid` を 4.10.1 から 5.38.1 へ上げたことで、両方が依存に入った状態になった。次にダイアログ・メニュー・ポップオーバーの類を作る人が、v0 のコードを真似て `@kobalte/core` を選ぶか、新しく `@ark-ui/solid` を選ぶか、参照するコードによって決まってしまう。
