...
## なぜ購読単位の `onClosed` では足りないか

接続プールは「リレーがレート制限でこの `REQ` を CLOSED した」と「ソケットが死んだ」を区別しなければならない。`RelaySubscriptionHandlers.onClosed(reason)` は両者を区別しない — どちらの場合でも呼ばれ、理由が文字列で渡されるだけである。

`RelayConnection` に **接続単位の** `onClose(listener: () => void): () => void` を足すことで、プールが購読の状態と接続の死を個別に観測できるようになる。

- 購読が relay から CLOSED を受けたなら、その購読だけを再試行する（バックオフに入る等）
- 接続が死んだなら、それにぶら下がる全購読を同時に失わせて、プール側で接続を交換する

この seam は **アダプタの薄さを壊さない**。`RelayConnection` アダプタは自分のソケットの状態を報告するだけで、再試行やバックオフといった方針を持たない。その判断は呼び出し側（プール）の責任である。
