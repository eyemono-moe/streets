...
## この ADR の射程（2026-08-06 追記）

**この決定がかかるのは Nostr に関連する部分だけである。** 理由は上記のとおり「NIP の変化が速く、追従のためにライブラリの更新を待つ経路を作りたくない」ことであり、**この理由は Nostr 以外には一切当てはまらない。**

Nostr と関係のない領域（外部入力の検証、日付、ルーティング、UI ユーティリティなど）では、ある程度有名でメンテナンスされているライブラリを**積極的に使ってよい**。手書きの型ガードを新しく書く前に、既存の依存で済まないかを見ること。

この追記は、実際に過度な一般化が起きたために足した。`src/core/deck/deck.ts` は localStorage の JSON を検証する型ガードを約 100 行手書きしていたが、**`valibot` は既にこのリポジトリの依存に入っている**（v0 の `src/features/Column/libs/deckSchema/v0.ts` が使っている）。ワイヤ形式そのもの（`NostrEvent` / `RelayFilter` / NIP-01 のメッセージ検証）は本 ADR のとおり自前で持つが、**それを包む永続化フォーマットの検証は Nostr の問題ではない。**
