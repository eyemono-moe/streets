...
### アンカー購読が「接続を握る」手段として REQ を悪用している（2026-08-05、実機で発覚）

`warmUpRouting` は、フェーズ①とフェーズ②の間で接続が落ちないよう、インデクサごとに `{ ids: [NEVER_MATCHING_ID] }` という**絶対にマッチしない filter** の購読を張っている（`bootstrap.ts`）。

**実機で、一部のリレーがこれを `blocked: filters must specify at least one kind` で拒否して CLOSED を返していることが分かった。**

**機能は壊れていない。** 接続を生かしているのは REQ ではなく `ConnectionPool` の**エントリ**であり（`#drop()` は `pooled.entries.size === 0` で発火する）、アンカーの `onClosed` は no-op なのでエントリは残る。つまり REQ が拒否されても接続は保たれる。

**しかしそれは偶然に寄りかかっている。** 我々が欲しいのは「接続の参照を 1 つ握る」ことだけで、REQ は**それを表現する API が他に無いから送っているだけ**である。帰結:

- 答えを一切必要としない REQ を毎回送っている（純粋な無駄）
- リレー側にエラーログとノイズを出させている
- **リレーが購読ではなく接続そのものを閉じる方針だった場合、ウォームアップが壊れる。** 今そうなっていないのは、たまたま各リレーがそこまで厳しくないからにすぎない

**対処案**: `ConnectionPool` に「REQ を伴わずに接続の参照だけ握る」口を足す（`retain(url): () => void` のような、エントリを 1 つ増やして解放関数を返すもの）。`#attachConnection` の再購読ループがそういうエントリを飛ばすようにする必要がある。小さく閉じた変更で、`NEVER_MATCHING_ID` という定数ごと消せる。
