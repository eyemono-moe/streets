...
| `event-requests.ts` / `profile-requests.ts` | **どちらのコアレッサも 1 バッチを 1 本のフィルタに全部詰め、分割しない。** A-2 の最終レビュー（2026-08-07、controller）で判明。`DeckColumn` は `<For each={items()}>` で最大 500 件（`MAX_ITEMS_PER_SECTION`）を仮想化なしに一度に描く。各 `NoteFull` が返信の親 1 + 引用 N の `EventView` を出すので、**1 カラムが settled した瞬間に 1 本の `fetchOnce({ ids: [...] })` が 1000 件規模の `ids` を持ちうる**。hex 64 文字 × 1000 ＝ 約 64KB で、NIP-11 の `limitation.max_message_length` を 16KB などに設定しているリレーは**メッセージごと拒否する**。拒否されると `fetchOnce` はタイムアウトまたは CLOSED で解決し、**そのバッチの id が全部 `isUnresolved` になる** —— 画面には「読み込めませんでした」が一斉に並ぶ。プロフィール側も同じ形（`authors` が数百件になりうる）。**A-2 以前から `profile-requests.ts` にあった形だが、A-2 が 2 本目のコアレッサを足して量を数倍にした。** 直し方は素直で、`pending` を固定長で切って複数の `fetchOnce` に分ける（上限値は NIP-11 の `max_message_length` から引くのが本筋だが、まず保守的な定数でよい）。**測る手段が無いのが本当の問題**という点で、初回描画 2 秒の未測定と同根 |
