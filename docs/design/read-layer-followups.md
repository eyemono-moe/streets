...
[仕様](../superpowers/specs/2026-08-11-note-rendering-design.md) 8 節が定める 3 問への回答。**3 問とも実鍵で v0（`/`）と v1（`/v1`）を並べて見ないと答えられない。** ローカルの docker リレーのシードは著者 3 人・短い定型文だけで、名前の長さも画像の縦横比も実データの分布を持たない。以下は何を見れば答えられるかだけを書く。**推測は書かない。**
