...
第1スライスの Critical はまさにここから生まれた（共有 store で `put` が `"duplicate"` を返す経路）。IndexedDB 水和が入れば store は読み取り層のシングルトンになるほかなく、そのとき全呼び出し箇所の移行が必要になる。**永続化の計画の冒頭で内部化する。**

### `seenRelays` の帰属が検証されていない

`event-store.ts` の `"duplicate"` 経路は、ペイロードを照合する前に `seenRelays` へリレーを記録する。悪意あるリレーは既知の ID を送るだけで、自分が配信していないイベントの提供者として記録される。

現在は無害（`seenRelays` に本番の読み手がいない）。**Outbox のリレーヒントや「どこで見たか」の UI がこれを読み始めた時点で実害になる。** 安価な対処: `"duplicate"` 経路の push を `computeEventId(unsigned) === event.id` で門番する。sha256 のみで schnorr を伴わないため、重複ごとの検証コスト（この方式を却下した理由）は発生しない。
