...
**採番についての注記。** この文書内の「後続 #」は 2 つの異なる採番系列を指す。23・25・30 行目および [ADR-0016:81](../adr/0016-routing-bootstrap.md) の「後続 #4」はこの文書が独自に振った古い採番で、永続化（IndexedDB 水和・`EventStore` の内部化）を指す。一方、117・125 行目の「後続 #4」「後続 #5」は [ADR-0023](../adr/0023-centralized-subscription-manager.md)「実装の段階」が定める採番（#1 Outbox ルーティング、#3 接続プール、#4 ローカルフィルタ照合、#5 REQ マージ）を指し、こちらは永続化とは無関係のスライスである。どちらも実在する採番であり、片方だけを訂正すると別の文書と食い違うため、ここでは両方をそのまま残し、混同しないようこの注記で区別する。
