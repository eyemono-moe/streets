...
**追記（2026-08-06 degraded-recovery-and-isolation Task 1）: 復帰側も同じ配線で閉じた。** `onDegradedChanged` は名前のとおり degraded 集合に入る瞬間だけでなく**出る瞬間**にも通知する — `#clearFailures`（クールダウン満了・`onOpen`・`retryNow()` の 3 経路すべてがここを通る）が、消す前に degraded だった URL について通知する。除外側と全く同じ `onDegradedChanged` フックと `DEGRADED_REPLAN_BATCH_MS` バッチ窓に相乗りするので、`SubscriptionManager` 側に復帰専用の新しい配線は要らなかった。**したがって、下の「小さいもの」表がかつて書いていた「除外の側で閉じたのと全く同じ形の隙間が、復帰の側に残っている」はもう成り立たない** — その行自体も解消済みとして更新済み。詳細は [ADR-0021](../adr/0021-reconnection-policy.md) の該当節を参照。
