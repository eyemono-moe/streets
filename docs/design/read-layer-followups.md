...
人間による実鍵検証（2026-08-05）では、この数値についての混乱は報告されていない。**しかし「混乱しなかった」ことは「意味が分かった」ことの証拠ではない** —— 理解できたとも、単に見なかった／気にしなかったとも取れる。ここは**推測で埋めない**: 人間が実際にこの数値を見て何を理解したかについて、確度のある記録は無い。これ自体が、仕様 7 節が「ユーザー向け翻訳は繰延」とした判断を弱めも強めもしない — 唯一得られた一次情報（実装者自身の体験）は「生の数値はソースを読まないと意味が取れない」であり、翻訳層の必要性を裏づける方向の弱い証拠ではある。
