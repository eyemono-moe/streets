...
sequenceDiagram
  ...
  Note over SUB: マージした購読では<br/>どのフィルタに当たったか<br/>リレーは教えてくれない
  SUB->>SUB: ローカル再マッチ<br/>（NIP-01 のフィルタ意味論）
...
| 再接続・バックオフ | 切断は `incomplete.unreachableRelays` として表面化するので、劣化するが隠れない（[ADR-0021](../adr/0021-reconnection-policy.md) は proposed） | 後続 #3 |
