...
import { RenderProvider } from "@streets/core/view/render-context";
import type { RenderContextValue } from "@streets/core/view/render-context";
import type { EventRenderer } from "@streets/core/view/renderer-registry";
...
/** 200ms バッチ窓や `fetchOnce` を経由せず、バッチ完了のタイミングをテストが直接制御する。 */
...
/**
 * JSX を介さず関数として直接呼び DOM ノードを検証する。`EventView` の
 * `createEffect` はマイクロタスクで走るので `vi.waitFor` で結果を待つ。
 */
const mount = (
...
describe("EventView", () => {
  ...
  it("store に無ければ要求して待ち、届いたら描画する", async () => {
    ...
    try {
      ...
      // バッチが片付く前に届く経路 (isUnresolved を経由せず check() で解決)。
      store.put(event, "wss://relay/");
      fake.settle();
  ...
  it("isUnresolved が真になったら「読み込めませんでした」を出す", async () => {
    ...
    try {
      ...
      await vi.waitFor(() => {
        // 捕まえる変異: isUnresolved を見ない (取得中と失敗の区別が
        // 付かなくなる)
        expect(
  ...
  it("無関係なバッチの完了では失敗へ倒さない", async () => {
    ...
    try {
      ...
      //
      // 捕まえる変異: `isUnresolved(id)` を確かめずに倒す (コアレッサは
      // id 単位で通知しないので、他人の取得完了だけで失敗表示に変わる)。
  ...
  it("未登録の kind でも、レンダラ集合が空でも fallback を描く", async () => {
    ...
    try {
      await vi.waitFor(() => {
        // 捕まえる変異: 未登録 kind のとき UnknownKind へ倒さず何も描かない
