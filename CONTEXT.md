...
**カラム**:
デッキを構成する縦1列。中身はブロックの組み合わせで、1つ以上のセクションを持つ。
_Avoid_: ペイン、タブ、ウィジェット、フィード

**ブロック**:
カラムの中身を組み立てる部品。ソースを受け取って自分のセクションを作り、描く。カラムの種類を知らず、見せ方の設定（密度・画像・「表示するもの」など）だけをカラムから受け取る。イベントを1件ずつレンダラに渡すもの（`EventList`）が汎用で、まとめる・集計する・取り出す・木にするものは専用のブロックにする。
_Avoid_: ウィジェット、パーツ

**セクション**:
ブロックが持つ、1つのソースから取ったイベントの並び。ソース・並び順を持つ。大半のカラムは1セクションだけを持ち、ユーザー詳細のように固定情報とリストを併せ持つカラムが複数セクションになる。カラム内で最後のセクションだけが無限スクロールする。
_Avoid_: パート、エリア
...
**コードとドキュメントの読者は、開発中の会話・PR・レビュー・設計セッションを一切知らない新規参加者である。**
その人が読んで意味が通らない文、または「知らない何か」を参照している文は、価値が無いのではなくその人には届かない。
届かない文はコードに置かない。
...
- **ADR 番号** —— `(ADR-0011)` の類。読む上で必要な内容は、参照ではなく内容そのものを 1 行で書く
- **行番号つきの参照** —— `subscription-manager.ts:260`、`仕様 7 節`。ファイル名とシンボル名だけなら書いてよい

コメントが「今このコードが何を守っているか」ではなく「そこへ至った経緯」を語り始めたら、行き先は ADR である。

「旧」「以前」が**外の世界に今も存在する形式**（NIP-10 の位置ベース、NIP-04 の項目、保存形式の互換）を指すなら、それは変更履歴ではなくコードが守っている制約なので書く。

### コメントの分量

**1 つのコメントブロックは 4 行以内**（`/**` と `*/` の行を含む）。1 行は日本語 40 字程度で折り返す。
原因を先に結論を後に置いた「〜なので〜する」の形にする。

4 行を超えてよいのは、**3 行に畳んだとき失われる情報を 1 文で具体的に言える**場合だけ。
「全部大事だから」「文脈が必要だから」は言えたことにならない。
壊れ方の連鎖（A が起きて B になり C を経て D で沈黙する）は、ほぼ常に結論の 1〜2 行に置き換えられる。

例外として、独立した事実が複数あるものと、値ごとに別の意図がある列挙は、事実の数だけ行を使ってよい。

### テスト名

`describe` / `it` の文字列にも同じ規約が及ぶ。**失敗時に必ず読まれる場所**なので、
タスク ID・レビュー参照・仕様の節番号・作業計画への参照を入れない。
...
`docs/design/` にはコメントの分量規則を適用しない。判断理由を残すための文書なので、理由そのものは削らない。
削るのは、同じ主張の繰り返し、前置きと再説明、会話の順序をなぞった構成である。
