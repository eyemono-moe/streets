...
test("通知カラムは自分宛だけを集め、自分の行動を出さない", async ({ page }) => {
  ...
  // 対照: 他人のリアクション (+) が出ている。`+` は `ReactionMark` がハート
  // アイコン (testid `reaction-like`) で描き文字列としては残らないので、
  // テキスト一致ではなくこのアイコンの testid で見る。**この主張が無いと、
  // 下の「自分のリアクションが出ない」はカラムが空でも通ってしまう。**
  await expect(column.getByTestId("reaction-like")).not.toHaveCount(0);
